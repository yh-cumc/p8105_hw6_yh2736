...
```{r, message = FALSE, warning = FALSE}
###
birth_weight_dataset = birth_weight_dataset %>% 
  mutate(
...
fit <-  lm(bwt ~ babysex + mrace, data = birth_weight_dataset) 

###Modify the displaying value of labs
babysex_labs <- c("Male", "Female")

names(babysex_labs) <- c("1", "2")

mrace_labs <- c("White", "Black", "Asian", "Puerto Rican")

names(mrace_labs) <- c("1", "2", "3", "4")

###draw the figure1 with lm(bwt ~ babysex + mrace)
figure1 <- birth_weight_dataset %>% 
...
birth_weight_dataset %>% 
  ...
  ggplot(aes(x = pred, y = resid)) + 
  geom_violin() +
  facet_grid(babysex ~ mrace, labeller = labeller(babysex = babysex_labs, 
                                                  mrace = mrace_labs)) +
  xlab("Prediction") +
  ylab("Residual") +
  theme(
